    ...
    try {
      ...
      if (name === 'get_financial_dashboard') {
        ...
        const { data: upcomingReminders } = await adminSupabase
          .from('reminders')
          .select('id, title, amount, due_date, urgency')
          .eq('user_id', userId)
          .order('due_date', { ascending: true })
          .limit(10);
        const { data: userCreditCards } = await adminSupabase
          .from('credit_cards')
          .select('id, card_name, card_limit, manual_invoice_amount, closing_day, due_day')
          .eq('user_id', userId);
