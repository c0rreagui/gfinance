...
async function autoProvisionItauData(supabaseClient: any, userId: string) {
  try {
    // 1. Verificando/populando saldos reais do Itaú
    const { data: existingBalances } = await supabaseClient.from('balances').select('id').eq('user_id', userId);
    ...
    // 2. Verificando/populando cartões reais
    const { data: existingCards } = await supabaseClient.from('credit_cards').select('id').eq('user_id', userId);
    if (!existingCards || existingCards.length === 0) {
      await supabaseClient.from('credit_cards').insert([
    ...
    // 3. Verificando/populando lembretes reais
    const { data: existingReminders } = await supabaseClient.from('reminders').select('id').eq('user_id', userId);
    if (!existingReminders || existingReminders.length === 0) {
      await supabaseClient.from('reminders').insert([
        { user_id: userId, title: 'Mensalidade Faculdade UNIP', amount: 522.43, due_date: new Date('2026-08-10').toISOString(), paid: true, urgency: 'medium' },
        { user_id: userId, title: 'Conta Sabesp Concessionária (Parc 1)', amount: 17.56, due_date: new Date('2026-08-15').toISOString(), paid: true, urgency: 'low' },
        { user_id: userId, title: 'Conta Sabesp Concessionária (Parc 2)', amount: 17.56, due_date: new Date('2026-08-15').toISOString(), paid: true, urgency: 'low' }
      ]);
    }

    // 4. Verificando/populando transações exatas do extrato do Itaú (01/07 a 07/08)
    const { data: existingTx } = await supabaseClient.from('transactions').select('id').eq('user_id', userId);
    if (!existingTx || existingTx.length === 0) {
      const realItauTransactions = [
        ...
        { user_id: userId, description: 'Depósito / Salário Recebido Itaú', amount: 1789.86, category: 'Salário', date: new Date('2026-07-05T09:00:00.000Z').toISOString(), icon: 'ArrowDownLeft' },
        ...
        { user_id: userId, description: 'Mensalidade Faculdade UNIP', amount: -522.43, category: 'Boleto', date: new Date('2026-07-10T15:00:00.000Z').toISOString(), icon: 'FileText' },
        { user_id: userId, description: 'Conta de Água Sabesp (Parcela 1/2)', amount: -17.56, category: 'Utilidades', date: new Date('2026-07-15T16:00:00.000Z').toISOString(), icon: 'Zap' },
        { user_id: userId, description: 'Conta de Água Sabesp (Parcela 2/2)', amount: -17.56, category: 'Utilidades', date: new Date('2026-07-15T16:05:00.000Z').toISOString(), icon: 'Zap' }
      ];
      await supabaseClient.from('transactions').insert(realItauTransactions);
    }
  } catch (err) {
...
export async function executeFinancialTool(
  ...
  // Auto-Provisionar dados reais do Itaú no Supabase se o banco estiver limpo
